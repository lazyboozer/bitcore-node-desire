...
To develop bitcore-node-desire:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-desire.git
...
We now will setup symlinks in `bitcore-node-desire` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-desire
ln -s ~/bitcoind-rpc-desire
```
...
cd devnode
mkdir node_modules
touch bitcore-node-desire.json
touch package.json
```

Edit `bitcore-node-desire.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-desire/bin/bitcore-node-desire start
```